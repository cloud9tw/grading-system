...
### 4. 🔄 CEEP 資料同步 (CEEP2 Data Sync)
- **自動化 Scraping**：整合 Playwright 技術，一鍵登入 `ceep2.tmu.edu.tw`。
- **DOPS 歸檔**：自動擷取 114 學年度 DOPS 評量表內容，同步至 Google Sheets 的 `CEEP_DOPS` 分頁。
...
- **後端**: Flask (Python)
- **前端**: Tailwind CSS, JavaScript (Vanilla)
...
- **`上下班打卡記錄`**: 存放所有簽到退數據。
- **`CEEP_DOPS`**: (自動生成) 存放從 CEEP 同步過來的 DOPS 分數。
- **`系統設定`**: 全球系統參數與預警設定。
  - **A 欄 (負面關鍵字)**: 若教學回饋命中此處詞彙，會自動發信給管理員。 (範例：`爛`, `差`, `態度惡劣`)
  - **B 欄 (排除日期)**: 指定不寄送缺席警報的日期。 (格式範例：`2024-05-01`)
    - *註: 每行填寫一筆日期，系統會自動跳過該日的未簽到通報。*
...
- **系統管理員**: 教師名單中權限設為 `admin` 者，可使用「管理中心」與「同步 CEEP」功能。
...
### 📌 評量基準範例
| 學員階段 | EPA/OPA 需求數量 | DOPS (CEEP代入) | 教學回饋表需求 |
| :--- | :--- | :--- | :--- |
| **一般實習生** | 基礎項目 10-15 筆 | 至少 1 筆 | 每個檢查室 1 筆 |
| **一年制學員 (R1)** | 進階站別 20+ 筆 | 至少 3 筆 | 每個檢查室 2 筆 |
| **二年制學員 (R2)** | 獨立執行 30+ 筆 | 至少 5 筆 | 每個檢查室 3 筆 |

---

## 🛠️ 管理員維護指南
1. **修改評量目標**：若需調整各階段所需數量，請至 `各類別EPA需求` 試算表修改對應欄位數值，排行榜積分與進度條會即時更新。
2. **新增學員類別**：在 `各類別EPA需求` 增加新的一欄（標題為類別名稱），系統即可自動識別該類別的學員。
