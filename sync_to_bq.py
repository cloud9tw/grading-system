...
def sync_all():
    print("🚀 Starting Full Data Sync (Sheets -> BigQuery)...")
    load_dotenv()
    ...
    # --- 1. SYNC ATTENDANCE ---
    print("\n[1/2] Syncing Attendance Records...")
    try:
        ...
        if len(vals) > 1:
            ...
            print(f"✅ Attendance Sync Success: {len(json_rows)} events.")
    except Exception as e: print(f"❌ Attendance Sync Failed: {e}")

    # --- 2. SYNC GRADING LOGS ---
    print("\n[2/2] Syncing Grading Logs (Growth Curve Data)...")
    try:
        ...
        if len(vals) > 1:
            ...
            for r in rows:
                if not r or not r[0]: continue
                # Mapping indexes based on: ['學員ID', '學員姓名', '站別', '檢查部位', '時間', '教師姓名' ... ]
                ts = parse_dt(r[4] if len(r) > 4 else '')
                if not ts: continue
                
                # Combine OPA items 1-8 for each group
                def join_opa(start_idx):
                    items = r[start_idx : start_idx+8]
                    return ",".join([str(x) for x in items if x])

                # Force student_id to be a clean string (convert 8.0 to '8')
            ...
            client.load_table_from_json(json_rows, table_id, job_config=job_config).result()
            print(f"✅ Grading Logs Sync Success: {len(json_rows)} entries.")
    except Exception as e: print(f"❌ Grading Sync Failed: {e}")
